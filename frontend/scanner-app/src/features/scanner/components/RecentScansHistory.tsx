...
export function RecentScansHistory({ signalRService, className }: RecentScansHistoryProps) {
  ...
  useEffect(() => {
    ...
    const handleScanProcessed = (transaction: ScanTransaction) => {
      setRecentScans(prev => [transaction, ...prev.slice(0, 99)]); // Keep last 100
    };
    ...
    <Card className={cn('w-full', className)}>
      ...
      <CardContent className="space-y-2">
        {recentScans.length === 0 ? (
          ...
          <VirtualList
            items={recentScans}
            itemHeight={expandedItems.size > 0 ? 120 : 80}
            height={400}
            renderItem={(transaction, _index, style) => (
            ...
            getItemKey={(_index, transaction) => transaction.id.toString()}
