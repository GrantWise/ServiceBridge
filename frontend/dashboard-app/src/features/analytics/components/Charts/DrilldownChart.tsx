...
export function DrilldownChart({ 
  ...
  const handleDrilldown = (item: DrilldownDataItem) => {
    if (currentLevel < drilldownLevels.length) {
      const nextLevel = drilldownLevels[currentLevel];
      if (nextLevel) {
        setCurrentLevel(prev => prev + 1);
        setBreadcrumbs(prev => [...prev, item.name || item.category || 'Unknown']);
  ...
  const handleBarClick = (data: any) => {
    if (data && currentLevel < drilldownLevels.length) {
